...
VS_OUTPUT VS(VS_INPUT input){
	...
	for (int i{}; i < 4; ++i)
	{
		float curBoneIdx = input.boneIdcs[i];
		float curBoneWeight = input.boneWeights[i];

		if (curBoneIdx > -1)
		{
			// Vertex is attached to bone:
			transformedPosition += curBoneWeight * mul(originalPosition, gBones[curBoneIdx]);
			transformedNormal += curBoneWeight * mul(input.normal, (float3x3)gBones[curBoneIdx]);
