...
void SpriteRenderer::UpdateBuffer(const GameContext& gameContext)
{
	...
	if (!m_pVertexBuffer || m_Sprites.size() > m_BufferSize)
	{
		...
		bufferDesc.ByteWidth = sizeof(SpriteVertex) * m_Sprites.size();

		D3D11_SUBRESOURCE_DATA data{};
		data.pSysMem = m_Sprites.data();

		Logger::LogHResult(gameContext.pDevice->CreateBuffer(&bufferDesc, &data, &m_pVertexBuffer), L"SpriteRenderer");
	}
	...
	gameContext.pDeviceContext->Map(m_pVertexBuffer, 0, D3D11_MAP::D3D11_MAP_WRITE_NO_OVERWRITE, 0, &subResource);
	memcpy(subResource.pData, m_Sprites.data(), m_Sprites.size() * sizeof(SpriteVertex));
                                   ...
                                   DirectX::XMFLOAT2 scale, float rotation)
{
	...
	for (unsigned int i = 0; i < techDesc.Passes; ++i)
	{
		...
		gameContext.pDeviceContext->DrawIndexed(1, 0, 0);
	}
}
