...
void PhysxProxy::onContact(const physx::PxContactPairHeader& , const physx::PxContactPair* pairs,
                           physx::PxU32 count)
{
	for (physx::PxU32 i = 0; i < count; ++i)
	{
		// ignore pairs when shapes have been deleted
		if (pairs[i].flags & (physx::PxContactPairFlag::eDELETED_SHAPE_0 |
			physx::PxContactPairFlag::eDELETED_SHAPE_1))
			continue;
		...
		{
			GameObject* contact = contactComponent->GetGameObject();
			GameObject* other = otherComponent->GetGameObject();

			if (pairs[i].events & physx::PxPairFlag::eNOTIFY_TOUCH_FOUND)
				contact->OnContact(contact, other, GameObject::ContactAction::ENTER);
			if (pairs[i].events & physx::PxPairFlag::eNOTIFY_TOUCH_LOST)
				contact->OnContact(contact, other, GameObject::ContactAction::LEAVE);
		}
	}
}
